...
def chi2_difference(obskey, obs, modkey, mod, tp=False, rtype='r012', corr=True, inc=""):
    """
    Calculates the chi2 difference between observations and model ratios given
    the fitting options.
    """
    obsratio, _, covinv = freq_fit.compute_ratios(
        obskey, obs, rtype, threepoint=tp
    )
    if not corr:
        covinv = np.identity(covinv.shape[0]) * covinv
    ...
    if inc == "exclude":
        for i in range(len(obsratio[0,:])):
            if obsratio[2, i] == 1 and obsratio[3, i] == 21:
                x[i] = 0
    chi2 = (x.T.dot(covinv).dot(x))/len(x)
    return chi2
